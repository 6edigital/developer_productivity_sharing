...
(function (global, factory) {
  ...
  function ioReaderCallback() {
    ...
    image.onload = function () {
      ...
      if (defaultOrientArr.indexOf(orient) > -1) {
        switch (orient) {
          case 3:
            // orientData.rotation = Math.PI / 180 * 180;
            drawSprite(this,{rotation:Math.PI / 180 * 180});
            break;
          case 6:
            // orientData.rotation = Math.PI / 180 * 90;
            drawSprite(this,{rotation:Math.PI / 180 * 90});
            break;
          default:
            // orientData.rotation = Math.PI / 180 * -90;
            drawSprite(this,{rotation:Math.PI / 180 * -90});
  ...
  function createImage(data) {
    var img = new Image();
    img.onload = function () {
      drawSprite(this,data);
    };
    img.src = imgSource;
  }

  function drawSprite(img,data) {
    var imgW = stageConfig.width,
        imgH = stageConfig.width * (img.height / img.width),
        sprite = PIXI.Sprite.fromImage(img.src,true,1);

    if (img.width> window.innerWidth) {
      sprite.width = imgW;
      sprite.height = imgH;
    }else if(data && data.width){
        console.log(data.width/img.width);
      sprite.scale.x =sprite.scale.y = data.width/img.width;
    }
    ...
    sprite.rotation = 0;
    if(data){
        if(data.x)sprite.position.x = data.x;
        if(data.y)sprite.position.y = data.y;
    ...
    // orientData = {
    //   x: orientData.x || sprite.position.x,
    //   y: orientData.y || sprite.position.y,
    //   width:  orientData.width || sprite.width,
    //   rotation: orientData.rotation || sprite.rotation
    // };

    sprite.orientData = data;
  ...
  function getCurrentSprite() {
    ...
    // 初始缩放
    current.initScale = this.scale.x;
  ...
  function onRotate(e) {
    ...
    if (current) {
      ...
      if (e.type === 'rotatemove') {
        current.rotation = lastRotation - (startRotation - e.rotation) * Math.PI / 180;
      }
    }
  }
  ...
  /**
   * 添加图片元素
   * @param {Object|String} fileInput 图片文件或地址
   * @param {String} type 图片分类
   * @param {Number} x x位置
   * @param {Number} y y位置
   * @param {Number} rotation rotation值
   */
  photoEditor.addSprite = function (params) {
    if (!params.fileInput || !params.type) {
      console.log('add image fail!');
      return;
    }

    imgSource = params.fileInput;
    imgType = params.type;
    ...
    typeof imgSource === 'string' ? createImage(data) : processImage();
  };
